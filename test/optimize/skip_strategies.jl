...
@testset "QN skip strategies" begin
    # Rosenbrock — a standard test where BFGS/L-BFGS should converge
    ...
    @testset "QN restart on line search failure" begin
        ...
        res = solve(
            prob,
            copy(x0),
            LineSearch(BFGS(), tight_hz),
            OptimizationOptions(maxiter = 200, max_restarts = 5),
        )
    ...
    @testset "L-BFGS memory not incremented on skip" begin
        ...
        res_skip =
            solve(prob, copy(x0), LineSearch(LBFGS(; skip = always_skip)), short_opts)
