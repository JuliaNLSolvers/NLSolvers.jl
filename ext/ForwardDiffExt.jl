...
function NLSolvers.ScalarObjective(autodiff::ForwardDiffAutoDiff, x; f = nothing, g = nothing, fg = nothing, fgh = nothing, h = nothing, hv = nothing, batched_f = nothing, param = nothing)
    sco = ScalarObjective(f, g, fg, fgh, h, hv, batched_f, param)
    _autodiff(autodiff, x, sco)
end
function _autodiff(::ForwardDiffAutoDiff, x, so::ScalarObjective)
    g_f = (g,x) -> let f = so.f
